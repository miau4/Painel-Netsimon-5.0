...
#
# O access.log do Xray NUNCA grava o UUID na linha, apenas o
# "email:" do cliente. O formato real de uma linha é:
...
#
# Filtra por "email: $user" (o UUID não aparece no access.log) e
# extrai o IP do campo 3 (formato real: IP:PORTA fica logo após o
# timestamp, antes de "accepted").
# -------------------------------------------------------
...
kick_xray_uuid() {
    ...
    # 1. Remove o cliente do config temporariamente (via lib, com lock)
...
while true; do
    ...
    while IFS='|' read -r user uuid exp pass limit; do
        ...
        # Remoção de expirados é responsabilidade do painel (menu
        # "Excluir Expirados" ou módulo do painel web).
        # ===================================================
